import { IPlayerMetadataResponse } from 'src/types/responses/IPlayerMetadataResponse';

export default {
    programme: {
        type: 'episode',
        pid: 'b008m7xk',
        image: { pid: 'p07xpnby' },
        media_type: 'audio_video',
        title: 'Shrek 2',
        short_synopsis:
      'Mike Myers and Cameron Diaz lead an all-star cast in the kingdom of Far Far Away.',
        medium_synopsis:
      'Animated sequel following the grumpy ogre (voiced by Mike Myers) and his bride as they head off to meet her parents. The occasion is marred by the wicked Fairy Godmother.',
        long_synopsis:
      'Animated sequel following the grumpy ogre and his bride as they head for the land of Far Far Away to meet her parents. As Shrek contends with his new in-laws, the wicked Fairy Godmother sets in motion a dastardly scheme to scupper his marriage to Fiona. And will any of them survive at the hands of dandy assassin Puss in Boots?\n\nFeaturing the voices of Mike Myers (Shrek), Eddie Murphy (Donkey), Cameron Diaz (Princess Fiona), Antonio Banderas (Puss in Boots), Julie Andrews (Queen), John Cleese (King), Rupert Everett (Prince Charming), Jennifer Saunders (Fairy Godmother), Aron Warner (Wolf), Kelly Asbury (Page/Elf/Nobleman/Nobleman\u0027s Son), Cody Cameron (Pinocchio/Three Pigs) and Conrad Vernon (Gingerbread Man/Cedric/Announcer/Muffin Man).',
        first_broadcast_date: '2007-12-25T16:40:00Z',
        display_title: { title: 'Shrek 2', subtitle: '' },
        ownership: {
            service: { type: 'tv', id: 'bbc_one', key: 'bbcone', title: 'BBC One' },
        },
        versions: [
            {
                canonical: 1,
                pid: 'b008m1w0',
                duration: 5100,
                types: ['Original version'],
            },
            { canonical: 0, pid: 'b05qq4zd', duration: 5100, types: ['Editorial'] },
            { canonical: 0, pid: 'b06tmfch', duration: 4920, types: ['Editorial'] },
            {
                canonical: 1,
                pid: 'm0026dm2',
                duration: 4980,
                types: ['Original version'],
            },
            {
                canonical: 0,
                pid: 'p01wyft3',
                duration: 5100,
                types: ['Dubbed Audio Described'],
            },
            {
                canonical: 0,
                pid: 'p04sm8p8',
                duration: 5100,
                types: ['Dubbed Audio Described', 'Editorial'],
            },
        ],
        links: [
            {
                type: 'related_site',
                title: 'IMDb: Shrek 2',
                url: 'http://www.imdb.com/title/tt0298148/',
            },
            {
                type: 'related_site',
                title: 'Rotten Tomatoes: Shrek 2',
                url: 'https://www.rottentomatoes.com/m/shrek_2/',
            },
            {
                type: 'related_site',
                title: 'Wikipedia: Shrek 2',
                url: 'https://en.wikipedia.org/wiki/Shrek_2',
            },
        ],
        supporting_content_items: [],
        categories: [
            {
                type: 'format',
                id: 'PT001',
                key: 'animation',
                title: 'Animation',
                narrower: [],
                broader: {},
                has_topic_page: false,
            },
            {
                type: 'genre',
                id: 'C00193',
                key: 'comedy',
                title: 'Comedy',
                narrower: [],
                broader: {},
                has_topic_page: false,
            },
            {
                type: 'genre',
                id: 'C00003',
                key: 'entertainmentandcomedy',
                title: 'Entertainment \u0026 Comedy',
                narrower: [],
                broader: {
                    category: {
                        type: 'genre',
                        id: 'C00001',
                        key: 'childrens',
                        title: 'Children\u0027s',
                        broader: {},
                        has_topic_page: false,
                    },
                },
                has_topic_page: false,
            },
            {
                type: 'format',
                id: 'PT007',
                key: 'films',
                title: 'Films',
                narrower: [],
                broader: {},
                has_topic_page: false,
            },
        ],
    },
} as IPlayerMetadataResponse;

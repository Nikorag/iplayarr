    ...
    describe('createNZBDownloadLink', () => {
        ...
        describe('parseEpisodeDetailStrings', () => {
            it('extracts number from title using regex', () => {
                const [title, series, episode] = Utils.parseEpisodeDetailStrings('Doctor Who: Series 3', '4', '1');
            ...
            it('falls back to default series number if no match', () => {
                const [title, series, episode] = Utils.parseEpisodeDetailStrings('Doctor Who', '4', '2');
                expect(title).toBe('Doctor Who');
                expect(series).toBe(4);
                expect(episode).toBe(2);
            });

            it('returns undefined for invalid series and episode values', () => {
                const [title, series, episode] = Utils.parseEpisodeDetailStrings('Doctor Who', 'SEVEN', 'TWO');
                expect(title).toBe('Doctor Who');
        ...
        describe('calculateSeasonAndEpisode', () => {
            describe('episodes', () => {
                it('series episode', async () => assertSeasonAndEpisode(m0029c0g, [ VideoType.TV, 1, 'Beyond Paradise', 3]));        
                it('roman numerals series', async () => assertSeasonAndEpisode(p00bp2rm, [ VideoType.TV, 5, 'Red Dwarf', 4 ]));        
            ...
            const assertSeasonAndEpisode = async (metadata: IPlayerMetadataResponse, expected: [ type: VideoType, episode?: number, episodeTitle?: string, series?: number ]) => {
                mockedEpisodeCacheService.getMetadata.mockResolvedValueOnce(metadata);
                await expect(Utils.calculateSeasonAndEpisode(metadata.programme)).resolves.toEqual(expected);
